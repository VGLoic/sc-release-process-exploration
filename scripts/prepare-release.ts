...
async function prepareRelease() {
  ...
  if (previousReleases.length === 0) {
    ...
    try {
      ...
      console.log(
        `✅ The first release ${INITIAL_RELEASE_NAME} has been created. You can now run \`yarn build\` to generate the artifacts to be distributed.`,
      );
    } catch (err) {
      console.error(
    ...
    const nextReleaseName = `v${lastRelease.semver.major}.${
      lastRelease.semver.minor + 1
    }.${lastRelease.semver.patch}`;
    // 4. we rename the `./releases/tmp` to `./releases/${nextReleaseName}`
    const renameResult = await toResult(
      fs.rename("./releases/tmp", `./releases/${nextReleaseName}`),
    );
    if (!renameResult.ok) {
      console.error(
    ...
    console.log(
      `✅ The next release ${nextReleaseName} has been created. You can now run \`yarn release:generate-delta && yarn build\` to generate the artifacts to be distributed.`,
    );
  }
}
